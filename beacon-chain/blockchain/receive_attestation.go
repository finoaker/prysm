...
func (s *Service) AttestationPreState(ctx context.Context, att *ethpb.Attestation) (*state.BeaconState, error) {
	ss, err := helpers.StartSlot(types.ToEpoch(att.Data.Target.Epoch))
	if err != nil {
		return nil, err
	}
...
func (s *Service) AttestationCheckPtInfo(ctx context.Context, att *ethpb.Attestation) (*pb.CheckPtInfo, error) {
	...
	return s.getAttCheckPtInfo(ctx, att.Data.Target, helpers.SlotToEpoch(types.ToSlot(att.Data.Slot)))
}

// VerifyLmdFfgConsistency verifies that attestation's LMD and FFG votes are consistency to each other.
func (s *Service) VerifyLmdFfgConsistency(ctx context.Context, a *ethpb.Attestation) error {
	return s.verifyLMDFFGConsistent(ctx, types.ToEpoch(a.Data.Target.Epoch), a.Data.Target.Root, a.Data.BeaconBlockRoot)
}
	...
	for {
		...
		case <-st.C():
			...
			for _, a := range atts {
				...
				if err := helpers.VerifySlotTime(uint64(s.genesisTime.Unix()), types.ToSlot(nextSlot), params.BeaconNetworkConfig().MaximumGossipClockDisparity); err != nil {
					continue
				}
...
func (s *Service) verifyCheckpointEpoch(c *ethpb.Checkpoint) bool {
	...
	if c.Epoch != prevEpoch.Uint64() && c.Epoch != currentEpoch.Uint64() {
		return false
	}
