...
func IsActiveValidator(validator *ethpb.Validator, epoch types.Epoch) bool {
	activationEpoch := types.ToEpoch(validator.ActivationEpoch)
	exitEpoch := types.ToEpoch(validator.ExitEpoch)
	return checkValidatorActiveStatus(activationEpoch, exitEpoch, epoch)
}
...
func Domain(fork *pb.Fork, epoch types.Epoch, domainType [bls.DomainByteLength]byte, genesisRoot []byte) ([]byte, error) {
	...
	forkEpoch := types.ToEpoch(fork.Epoch)
	if epoch < forkEpoch {
...
func IsEligibleForActivationQueue(validator *ethpb.Validator) bool {
	activationEligibilityEpoch := types.ToEpoch(validator.ActivationEligibilityEpoch)
	return isEligibileForActivationQueue(activationEligibilityEpoch, validator.EffectiveBalance)
}
...
func IsEligibleForActivation(state *stateTrie.BeaconState, validator *ethpb.Validator) bool {
	...
	activationEligibilityEpoch := types.ToEpoch(validator.ActivationEligibilityEpoch)
	activationEpoch := types.ToEpoch(validator.ActivationEpoch)
	return isEligibleForActivation(activationEligibilityEpoch, activationEpoch, finalizedEpoch)
}
...
func IsEligibleForActivationUsingTrie(state *stateTrie.BeaconState, validator *stateTrie.ReadOnlyValidator) bool {
	...
	return isEligibleForActivation(validator.ActivationEligibilityEpoch(), validator.ActivationEpoch(), types.ToEpoch(cpt.Epoch))
}
